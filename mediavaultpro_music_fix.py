...
try:
    from mediavault_music_integration import integrate_music_player
    HAS_MUSIC_INTEGRATION = True
    print("✓ Music player integration available")
except ImportError as e:
    print(f"Music player integration not available: {e}")
    HAS_MUSIC_INTEGRATION = False
...
class MetadataEditorApp(tk.Tk):
    def __init__(self):
        ...
        self.music_integration = None
        if HAS_MUSIC_INTEGRATION:
            try:
                self.music_integration = integrate_music_player(self)
                print("✓ Music player integration initialized successfully")
            except Exception as e:
                print(f"✗ Error initializing music integration: {e}")
    ...
    def rotate_video(self, angle: int) -> None:
        ...
        # Check if we have a video file (either current_file or current_video_path)
        is_video = False
        if self.current_file and self.current_file.lower().endswith(tuple(VIDEO_EXTENSIONS)):
            is_video = True
        ...
        if self.video_playing:
            ...
            if has_cv2 and video_path:
                # Force refresh the video display with rotation
                if hasattr(self, 'video_canvas') and self.video_canvas:
                    # Clear the canvas first
                    self.video_canvas.delete("all")
                
                if hasattr(self, 'display_video_thumbnail_simple'):
                    print(f"DEBUG: Refreshing video thumbnail with rotation {self.video_rotation}°")
                    self.display_video_thumbnail_simple(video_path)
                elif hasattr(self, 'display_video_thumbnail'):
                    print(f"DEBUG: Refreshing video thumbnail with rotation {self.video_rotation}°")
    ...
    def _build_widgets(self) -> None:
        ...
        self.tree = ttk.Treeview(
            ...
            show="headings",
            selectmode="extended",  # Enable multi-select for music integration
            height=25,
        )
    ...
    def on_search_change(self, *args):
        """Handle live search as user types with immediate incremental filtering."""
        ...
        # Immediate jump-to functionality for short queries (1-3 characters)
        if len(q) <= 3:
            self.jump_to_first_match(q)
            # Still schedule filtered search after short delay for longer queries
            self.search_timer = self.root.after(200, self.perform_search)
        else:
            # For longer queries, perform immediate filtering
            self.perform_search()
    
    def jump_to_first_match(self, query):
        """Jump to first alphabetically matching item in current view."""
        try:
            query_lower = query.lower()
            
            # Search through current tree items
            for item_id in self.tree.get_children():
                item_values = self.tree.item(item_id)['values']
                if item_values:
                    item_text = str(item_values[0]).lower()  # File name is first value
                    if item_text.startswith(query_lower):
                        # Found match - select and focus
                        self.tree.selection_set(item_id)
                        self.tree.focus(item_id)
                        self.tree.see(item_id)  # Ensure it's visible
                        print(f"Jumped to: {item_values[0]}")
                        break
        except Exception as e:
            print(f"Error in jump_to_first_match: {e}")
    ...
    def on_tree_double_click(self, event):
        """Handle double-click on tree items: drill down on group headers, play songs otherwise."""
        item = self.tree.selection()
        if not item:
            return
        item_id = item[0]
        ...
        # If already drilled down, always play the selected song
        if self.navigation_state["is_drilled_down"]:
            self.play_song_from_drill_down(item_id)
            return

        # Group header handling: only drill down for supported music sub-filters
        if item_id.startswith("group_"):
            supported = ["Music: Artist", "Music: Album", "Music: Genre", "Music: Mood", "Music: Year"]
            if current_filter.get() in supported:
            ...
            # For other filters, ignore group header double-clicks
            return

        # Non-group items represent individual songs: play with queue management
    ...
    def play_song_from_main_view(self, item_id):
        ...
        try:
            ...
            if sys.platform == "darwin":  # macOS - Use smart queue management
                ...
                # Check current filter to determine behavior
                current_filter = getattr(self, 'filter_var', None)
                filter_type = current_filter.get() if current_filter else "All"
                
                if is_playing or self.queue_manager.has_active_queue():
                    queue_size = len(self.queue_manager.local_queue)
                    
                    # For "All" filter, automatically append to queue without dialog
                    if filter_type == "All" and queue_size > 0:
                        print(f"Auto-appending '{file_name}' to queue (All filter)")
                    ...
                    # For other filters, show the dialog as before
                    if queue_size > 0:
    ...
    def play_song_from_drill_down(self, item_id):
        ...
        try:
            ...
            if sys.platform == "darwin":  # macOS - Use smart queue management
                ...
                # Check current filter to determine behavior (consistent with main view)
                current_filter = getattr(self, 'filter_var', None)
                filter_type = current_filter.get() if current_filter else "All"
                ...
                if is_playing or self.queue_manager.has_active_queue():
                    ...
                    # For "All" filter, automatically append to queue without dialog (consistent behavior)
                    if filter_type == "All" and queue_size > 0:
                        print(f"Auto-appending '{file_name}' to queue (All filter - drill-down)")
                        success = self.queue_manager.smart_add_to_queue(file_path, file_name, self)
                        if not success:
                            print("Smart queue failed, falling back to regular play")
                            self.launch_music_player(file_path, file_name)
                    ...
                    # For other filters, show the same dialog as main view
                    if queue_size > 0:
                        from tkinter import messagebox
                        user_choice = messagebox.askyesnocancel(
                            "Add Song to Queue",
                        ...
                        if user_choice is None:  # Cancel
                            ...
                            print(f"Adding '{file_name}' to existing queue (drill-down)")
                            success = self.queue_manager.smart_add_to_queue(file_path, file_name, self)
                            if not success:
                                print("Smart queue failed, falling back to regular play")
                                self.launch_music_player(file_path, file_name)
                    ...
                    # Use smart queue management for seamless adding
                    print(f"Using smart queue management for '{file_name}' (drill-down)")
                    success = self.queue_manager.smart_add_to_queue(file_path, file_name, self)
                    if not success:
                        print("Smart queue failed, falling back to regular play")
                        self.launch_music_player(file_path, file_name)
...
'''
    ...
    def load_record(self, file_name: str):
        ...
        if media_type == "music":
            ...
            # Prefer TkVideoPlayer-based player with audio and proper timing
    ...
    def setup_image_preview(self, img_path: Path):
        ...
        # Clean up any existing preview state (video, music, image)
        self.stop_video()
        ...
        if getattr(self, 'video_canvas', None):
            try:
                self.video_canvas.destroy()
            except Exception:
                pass
        ...
        if getattr(self, 'video_controls', None):
            try:
                self.video_controls.destroy()
            except Exception:
                pass
            # Recreate a fresh, empty video_controls for later use
            self.video_controls = ttk.Frame(self.preview_frame)

        if getattr(self, 'music_controls', None):
            try:
        ...
        if getattr(self, 'image_frame', None):
            try:
        ...
        if getattr(self, 'preview_lbl', None):
            try:
        ...
        # Create a fresh preview label and image controls each time
        self.preview_lbl = ttk.Label(self.preview_frame)
        # Show a placeholder immediately to avoid empty space while loading
        if getattr(self, 'default_img', None):
            self.preview_lbl.configure(image=self.default_img)
        self.preview_lbl.pack(anchor="center")
        ...
        # Image info
        ttk.Label(self.image_controls, text=f"Image: {img_path.name}").pack(side="right", padx=5)

        # Pack the image frame below the preview
    ...
    def setup_video_preview(self, video_path: Path):
            ...
            try:
                ...
                for frame_num in range(30):
                    ...
                    if ret and temp_frame is not None:
                        # Enhanced frame viability detection
                        gray_frame = cv2.cvtColor(temp_frame, cv2.COLOR_BGR2GRAY)
                        mean_brightness = cv2.mean(gray_frame)[0]
                        
                        # Check for both brightness and variance (avoid completely uniform frames)
                        _, std_dev = cv2.meanStdDev(gray_frame)
                        variance = std_dev[0][0]
                        
                        # Frame is viable if it has sufficient brightness AND variance
                        if mean_brightness > 15 and variance > 10:
                            frame = temp_frame
                            frame_found = True
                            print(f"Found good frame at position {frame_num} (brightness: {mean_brightness:.1f}, variance: {variance:.1f})")
                            break
        ...
        external_button.pack(side="left", padx=5)
        
        # Add rotation buttons (similar to photo interface)
        ttk.Button(self.video_controls, text="Rotate Left",
                  command=lambda: (print("DEBUG: Rotate Left clicked"), self.rotate_video(-90))
        ).pack(side="left", padx=5)

        ttk.Button(self.video_controls, text="Rotate Right",
                  command=lambda: (print("DEBUG: Rotate Right clicked"), self.rotate_video(90))
        ).pack(side="left", padx=5)

        ttk.Button(self.video_controls, text="Save Rotation",
                  command=lambda: (print("DEBUG: Save Rotation clicked"), self.save_rotated_video())
        ).pack(side="left", padx=5)
    ...
    def setup_embedded_video_player(self, video_path: Path):
        ...
        try:
            # Set up video canvas for inline playback
            if has_cv2:
            ...
            # Simple controls
            ttk.Button(self.video_controls, text="▶️ Play Video", command=self.play_video).pack(side="left", padx=5)
            ...
            # Add rotation buttons (similar to photo interface)
            ttk.Button(self.video_controls, text="Rotate Left", command=lambda: (print("DEBUG: Rotate Left clicked (simple)"), self.rotate_video(-90))).pack(side="left", padx=5)
            ttk.Button(self.video_controls, text="Rotate Right", command=lambda: (print("DEBUG: Rotate Right clicked (simple)"), self.rotate_video(90))).pack(side="left", padx=5)
            ttk.Button(self.video_controls, text="Save Rotation", command=lambda: (print("DEBUG: Save Rotation clicked (simple)"), self.save_rotated_video())).pack(side="left", padx=5)
    ...
    def display_video_thumbnail_simple(self, video_path: Path):
        ...
        try:
            ...
            # Try to get a viable frame from first 30 frames with enhanced detection
            for frame_num in range(30):  # Try first 30 frames instead of 5
                cap.set(cv2.CAP_PROP_POS_FRAMES, frame_num)
                ret, frame = cap.read()
                ...
                if ret and frame is not None:
                    ...
                    # Frame is viable if it has sufficient brightness AND variance
                    if mean_brightness > 15 and variance > 10:  # Enhanced detection criteria
                        # Convert BGR to RGB
                        frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                        ...
                        # Display in video canvas if available AND this is actually a video file
                        is_video_file = str(video_path).lower().endswith(tuple(VIDEO_EXTENSIONS))
                        if hasattr(self, 'video_canvas') and self.video_canvas and is_video_file:
                            self.video_canvas.delete("all")
                            self.video_canvas.create_image(VIDEO_MAX[0]//2, VIDEO_MAX[1]//2, 
                                                         image=self.tk_img, anchor='center')
                            self.video_canvas.image = self.tk_img  # Keep reference
                        else:
                            self.preview_lbl.configure(image=self.tk_img, text="")
                            self.preview_lbl.pack(anchor="center")
                        thumbnail_created = True
    ...
    def on_play_all(self):
        """Handle Play All button - now uses enhanced music player when available."""
        try:
            # Get current songs list
            current_songs = []
            ...
            if not current_songs:
                print("No songs to play")
                messagebox.showinfo("Play All", "No songs found to play with current filter.")
                return
            ...
            if not playlist:
                print("No playable songs found")
                messagebox.showwarning("Play All", "No playable songs found.")
                return
            
            # Use enhanced music player if available
            if hasattr(self, 'music_integration') and self.music_integration and HAS_MUSIC_INTEGRATION:
                try:
                    from pygame_player_enhanced_integrated import TransferMode
                    
                    # Show music player
                    self.music_integration.show_music_player()
                    
                    # Get filter description for source tracking
                    filter_desc = self.music_integration.get_current_filter_description()
                    
                    # Check if there's existing playlist and prompt user
                    if (self.music_integration.music_player and 
                        len(self.music_integration.music_player.queue) > 0):
                        
                        user_choice = messagebox.askyesnocancel(
                            "Music Player Queue",
                            f"Music player has {len(self.music_integration.music_player.queue)} songs.\n\n"
                            f"Would you like to:\n"
                            f"• Yes: Replace playlist with {len(playlist)} new songs\n"
                            f"• No: Add {len(playlist)} songs to playlist\n"
                            f"• Cancel: Keep current playlist unchanged",
                            icon="question"
                        )
                        
                        if user_choice is None:  # Cancel
                            return
                        elif user_choice is False:  # No - Append
                    ...
                    success = self.music_integration.music_player.add_songs_from_external(
                        playlist, mode, filter_desc
                    )
                    
                    if success:
                        print(f"✓ Added {len(playlist)} songs to music player")
                        messagebox.showinfo("Play All", f"Added {len(playlist)} songs to music player!")
                        return
                    else:
                        print("✗ Failed to add songs to music player, falling back to system player")
                        
                except Exception as e:
                    print(f"Music player error: {e}, falling back to system player")
                    
            # Fallback: inform user that enhanced music player is not available
            messagebox.showinfo(
                "Play All", 
                f"Found {len(playlist)} songs.\n\n"
                "Enhanced music player not available. "
                "Original system music integration would be used here."
            )
            
        except Exception as e:
            print(f"Error in on_play_all: {e}")
            import traceback
            traceback.print_exc()
            messagebox.showerror("Error", f"Error playing songs: {e}")

    # --------------------------------------------------------------------
    # Shutdown
    # --------------------------------------------------------------------
    # ========== MUSIC PLAYER INTEGRATION HELPER METHODS ==============
    
    def get_song_at_index(self, index):
        """Get song data for the item at the specified index (used by music integration)."""
        try:
            # Get current results based on filter state
            current_results = self.get_current_filtered_results()
            
            if 0 <= index < len(current_results):
                song_row = current_results[index]
                file_path = self.get_music_file_path(song_row)
                song_name = song_row.get("file_name", "Unknown")
                return (file_path, song_name)
            return None
        except Exception as e:
            print(f"Error getting song at index {index}: {e}")
            return None
            
    def get_current_filtered_results(self):
        """Get the current filtered results for music integration."""
        try:
            if self.navigation_state["is_drilled_down"]:
                return self.navigation_state["category_songs"] or []
            else:
                # Get current filter results
                current_filter = getattr(self, 'filter_var', None)
                if current_filter:
                    filter_type = current_filter.get()
                    search_query = getattr(self, 'search_var', tk.StringVar()).get()
                    return self.db.search_filtered(search_query, filter_type)
                return []
        except Exception as e:
            print(f"Error getting current filtered results: {e}")
            return []
    ...
    def quit(self):
        ...
        # 1. Clean up music player integration
        if hasattr(self, 'music_integration') and self.music_integration:
            try:
                self.music_integration.cleanup()
                print("✓ Music player integration cleaned up")
            except Exception as e:
                print(f"Music integration cleanup error: {e}")

        # 2. Clean up queue manager and current temp playlists
        if hasattr(self, 'queue_manager') and self.queue_manager:
            try:
                if hasattr(self.queue_manager, 'current_temp_playlist') and self.queue_manager.current_temp_playlist:
        ...
        # 3. Stop any playing video or audio
        try:
            self.stop_video()
            print("✓ Video stopped")
        except Exception as e:
        ...
        # 4. Kill any external video process
        if hasattr(self, 'current_video_process') and self.current_video_process:
            try:
                self.current_video_process.terminate()
                self.current_video_process.wait(timeout=2)
        ...
        # 5. Kill any external audio process  
        if hasattr(self, 'current_audio_process') and self.current_audio_process:
            try:
                self.current_audio_process.terminate()
                self.current_audio_process.wait(timeout=2)
